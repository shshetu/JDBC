...
public class ProductPurchaseDaoImp implements ProductPurchaseDao {
    ...
    @Override
    public ProductPurchase getProductPurchaseByProductCode(String productCode) {
        ProductPurchase pr = null;
        String sql = "select * from productpurchase where product_code = ?";
        try {
            PreparedStatement pstm = conn.prepareStatement(sql);
            pstm.setString(1, productCode);
            ResultSet rs = pstm.executeQuery();
            while(rs.next()){
            pr = new ProductPurchase(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getDouble(4), rs.getInt(5), rs.getDouble(6), rs.getDate(7), new ProductCategory(rs.getInt(8)));
            }
        } catch (SQLException ex) {
            Logger.getLogger(ProductPurchaseDaoImp.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public List<ProductPurchase> getProductPurchase() {
        ...
        try {
            ...
            ResultSet rs = pstm.executeQuery();
            while(rs.next()){
            ProductPurchase ps = new ProductPurchase(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getDouble(4), rs.getInt(5),rs.getDouble(6), rs.getDate(7),new ProductCategory(rs.getInt(8)));
            list.add(ps);
            }
        } catch (SQLException ex) {
            Logger.getLogger(ProductPurchaseDaoImp.class.getName()).log(Level.SEVERE, null, ex);
